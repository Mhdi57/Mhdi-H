...
public class KkuSystem implements OptionalReserveTasks, FileNames, ReserveTasks {
	...
	public void showCLIMenu() {
		...
		OrderTime T=new OrderTime();
		OrderDate D=new OrderDate();
		...
		while(true) {
			...
			case "1":
				...
				T.TIME();
				st=this.reserveBlock(new ReserveBlock(p,D.getDate(),T.getTime(),r));
				}
				catch(Exception e) {
			...
			case "6":
				...
				st=this.removeBlock(new ReserveBlock(null,D.getDate(),T.getTime(),r));
				break;
